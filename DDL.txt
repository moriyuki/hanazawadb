...
-- dt_pc_user_date
drop table dt_pc_user_date cascade;

create table dt_pc_user_date (
  pud_date date
...
drop table dt_user_date cascade;

create table dt_user_date (
  ud_us_id integer not null
  , ud_date date default ('now'::text) not null
  , ud_comment character varying
  , constraint dt_user_date_PKC primary key (ud_us_id,ud_date)
) ;

-- dt_user_soft	 ユーザとソフトの関係を管理
drop table dt_user_soft cascade;

create table dt_user_soft (
  usf_us_id integer not null
  , usf_sf_id integer not null
  , usf_comment character varying
  , constraint dt_user_soft_PKC primary key (usf_us_id,usf_sf_id)
) ;

-- mt_pc	 pc管理マスタテーブル
drop table mt_pc cascade;
